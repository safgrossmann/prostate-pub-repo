...
#--------------------------------------------------------------------------------------------------------------------------
# Name		: runLCMFiltering_by_sample_project.sh
# Author	: Mathijs A. Sanders
# Version	: 1.03
# Copyright	: None
# Description	: runLCMFiltering.sh -pt project_IDs -st samples_tumour_csv -pn project_IDs -sn samples_normal_csv -o output_directory [-f 4] [-r] [-s snp_database] [-g reference_genome] [-h]
#--------------------------------------------------------------------------------------------------------------------------
...
annotateVariants() {
	...
	bsub -q normal -J "${TNM}1" -o ${OUTPUT_DIR}/log/log1.%J -e ${OUTPUT_DIR}/log/err1.%J -n2 -R"select[mem>5000] span[hosts=1] order[-slots] rusage[mem=5000]" -M5000 bash -e /lustre/scratch116/casm/cgp/users/ms44/scripts/annotateVariantsTumorNormal_speedup.sh ${OUTPUT_DIR}/${NM} ${BAM} ${CONTROL_BAM}
	
	sleep 5
        cd ${OUTPUT_DIR}
	
	#Mathijs submits to normal queue but several of my jobs die after hitting the run time limit - This is the only change I made to this script. One job dies because of mem limit - 4G done by Mathijs
	bsub -w "done(${TNM}1)" -q long -J "${TNM}2" -o ${OUTPUT_DIR}/log_sec/log2.%J -e ${OUTPUT_DIR}/log_sec/err2.%J -n1 -R"select[mem>8500] order[io] rusage[mem=8500]" -M8500 /software/jre1.8.0_74/bin/java -Xmx8G -cp /lustre/scratch116/casm/cgp/users/ms44/tools/javaApps/additionalBamStatistics/:/lustre/scratch116/casm/cgp/users/ms44/tools/htsjdk.jar additionalBamStatistics $OUTPUT_NM $BAM $REFERENCE $SNP_DB ${FNM}_second_filter.txt &
...
SNP_DB='/lustre/scratch116/casm/cgp/users/ms44/src/dbsnp/common_all_20170710.vcf.gz'
...
then
	echo "Error: Please provide an integer value as the fragment threshold"
	exit -1
fi
...
then
	...
	awk -v STR_PT=$PROJECT_TUMOUR_IDS -v STR_ST=$SAMPLE_TUMOUR_IDS -v STR_PN=$PROJECT_NORMAL_IDS -v STR_SN=$SAMPLE_NORMAL_IDS -v OUTPUT_DIR=$OUTPUT_DIR -v REF=$REFERENCE -v SNP=$SNP_DB 'BEGIN{n=split(STR_PT,apt,","); split(STR_ST,ast,","); split(STR_PN, apn, ","); split(STR_SN,asn, ","); for(i=1; i<=n ; i++){print apt[i] " " ast[i] " " apn[i] " " asn[i] " " REF " " SNP " " OUTPUT_DIR}}' | xargs -n1 -P0 -I {} bash -c 'annotateVariants {}'
